...
class RidgeRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        """
        m, d = X.shape
        lam_vec = np.full((d,), np.sqrt(self.lam_))
        I_zero = np.zeros(d)
        if self.include_intercept_:
            X = np.insert(X, 0, 1, axis=1)
            lam_vec = np.insert(lam_vec, 0, 0)
            I_zero = np.insert(I_zero, 0, 0)
        I_lam = np.diag(lam_vec)
        X_lam = np.concatenate((X, I_lam), axis=0)
        y_lan = np.concatenate((y, I_zero), axis=0)
        self.coefs_ = pinv(X_lam) @ y_lan
